  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws ssm put-parameter \
          --name "/${var.prefix}/${var.env}/bedrock/prompts/${local.prompt_id}/versions/$(date +%Y%m%d%H%M%S)" \
          --type "String" \
          --value "$VERSION_NUM" \
          --overwrite || true
    EOT
...
resource "null_resource" "system_prompt_version_ssm_param" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      aws ssm put-parameter \
          --name "/${var.prefix}/${var.env}/bedrock/prompts/${local.system_prompt_id}/versions/$(date +%Y%m%d%H%M%S)" \
          --type "String" \
          --value "$VERSION_NUM" \
          --overwrite || true
    EOT
